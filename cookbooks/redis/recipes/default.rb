...
execute "copy launchagent config" do
  command 'cp $(brew --prefix redis)/homebrew.mxcl.redis.plist ~/Library/LaunchAgents'
end

execute "load launchdaemon" do
  command "launchctl load -w ~/Library/LaunchAgents/homebrew.mxcl.redis.plist"
  not_if "launchctl list | grep redis"
end
